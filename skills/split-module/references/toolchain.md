...
| **mypy** or **pyright** (one required; `preflight.sh --install` installs mypy) | Type gate, run through `type_baseline.py` as a **ratchet**: pre-existing errors are recorded once from committed HEAD and grandfathered; only new errors fail. Name resolution needs no annotations, so even an untyped repo gets undefined-name detection at every site, which is the error a bad move leaves behind (bare name after de-qualification, dropped import, self-import). | `pip install mypy` or `pip install pyright` | `mypy --version` / `pyright --version`; `type_baseline.py status` | pyright is faster on very large packages; set `REFACTOR_TYPE_CMD` to scope or tune (e.g. `mypy --follow-imports=silent app/core`). |
